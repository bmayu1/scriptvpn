...
echo -e "              (Version 1.3.5) - by: @bumiayuvpn"
...
figlet -k BMAYU | awk '{gsub(/./,"\033[3"int(rand()*5+1)"m&\033[0m")}1' && figlet -k Hysteria | awk '{gsub(/./,"\033[3"int(rand()*5+1)"m&\033[0m")}1'
echo "───────────────────────────────────────────────────────────────────────•"
...
cat <<EOF
[Unit]
Description=FN-AIO Hysteria Service @fn_project
After=network.target
[Service]
User=root
Group=root
...
clear
echo ""
figlet -k FN-AIO | lolcat && figlet -k Hysteria | lolcat
echo -e "\e[1;34m************************************"
...
---------------------
(Version 1.3.5)
script by: @bm_project
EOF
chmod +x /etc/hysteria/client/config.json
echo ""
figlet -k BM-AIO | lolcat && figlet -k Hysteria | lolcat
echo -e "\e[1;36m----------------------"
